...
but they do not see virtual UDP payloads or service/path labels.

## Encrypted Relay Routing

Secure Gatherlink does not support plaintext routing. Relays must never depend
on plaintext `service_id`, `path_id`, `route_id`, endpoint address, tenant name,
or policy label inside a data packet. Data-plane routing through untrusted peers
requires a hop-authenticated relay envelope.
...
- `route_id`: derived from `service_id` plus authenticated config/control
  context when a v1 compatibility view needs it
