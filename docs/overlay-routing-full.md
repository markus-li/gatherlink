# Overlay Routing Full Design Notes
...
Roles are assigned per service/path, not globally.
...
The same node can be an entry for one service, a relay for another, and an exit
for a third.
...
The transit node must not need to understand LAN policy. It only needs to know
whether transit is allowed for that service/route and who the next hop is.

## Future protocol/control concepts

Leave design space for:
