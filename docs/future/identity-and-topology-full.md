# Identity And Topology Lifecycle Full Design Notes
...
than TOFU. WireGuard keeps trust explicit with configured peer public keys;
Gatherlink should keep that clarity while allowing a per-deployment or per-fleet
trust root to sign topology, roles, revocation, and helper permissions.
...
They should be single-use, expiring, scoped to intended node/fleet/role, redeemed
into signed topology/provisioning state, and audited.
