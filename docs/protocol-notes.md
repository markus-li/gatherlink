...
relay must not need plaintext `service_id`, `path_id`, `route_id`, endpoint
address, tenant name, or policy name to forward data.
...
- `route_id`: derived after decrypt from `service_id` plus authenticated
  config/control context when a v1 compatibility view needs it
...
decryption by mapping `receiver_index` to `session_id`, deriving `route_id`
from `service_id` plus authenticated control/config context, and deriving
